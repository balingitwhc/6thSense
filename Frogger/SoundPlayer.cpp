...
bool SoundPlayer::isPlaying(const std::string& soundName) const {
    // Check if the sound with the given name exists and if it's currently playing
    auto statusIt = m_soundStatus.find(soundName);
    if (statusIt != m_soundStatus.end() && statusIt->second) {
        return true;
    }
    return false;
}